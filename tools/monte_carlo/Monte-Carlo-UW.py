...
def evaluate(param_values,k):
	...
	m=len(param_values)
	for j in range(0,m):
		Newdbpath=sys.path[0]+'/db_io/dbs/4th/Method_of_Morris'+str(k)+'.db'
		...
		if len(param_names[j])==4:
			...
			con.commit()
		con.close()
	NewConfigfilePath=sys.path[0]+'/temoa_model/config_sample_4th'+str(k)
	copyfile(sys.path[0]+'/temoa_model/config_sample',NewConfigfilePath)
	with open(sys.path[0]+'/temoa_model/config_sample', 'r') as file:
		data = file.readlines()
	data[13]='--input=db_io/dbs/4th/Method_of_Morris'+str(k)+'.db'
	data[20]='--output=db_io/dbs/4th/Method_of_Morris'+str(k)+'.db'
	with open(NewConfigfilePath, 'w') as file:
		file.writelines(data)
	os.system('python temoa_model/ --config=temoa_model/config_sample_4th'+str(k))
	print k
...
problem = read_param_file(sys.path[0]+'/Monte-Carlo-4th.txt')
...
for k in range(0,Number_of_Simulations):
	...
	for i in range(1,problem['num_vars']):
		if problem['groups'][i]==problem['groups'][i-1]:
		   ...
		   param_values[k,i]=problem['bounds'][i][0]+lhd[k][c]*(problem['bounds'][i][1]-problem['bounds'][i][0])
	Newdbpath=sys.path[0]+'/db_io/dbs/4th/Method_of_Morris'+str(k)+'.db'
	copyfile(sys.path[0]+'/db_io/dbs/4th/Method_of_Morris.db',Newdbpath)
...
with open('MonteCarlo4th.csv', 'w') as f:
